...
def create_event(user_id, event_name, event_type, event_date, event_time, event_location, event_description):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""
        INSERT INTO events 
        (event_user_id, event_name, event_type, event_date, event_time, event_location, event_description)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (user_id, event_name, event_type, event_date, event_time, event_location, event_description))
    conn.commit()
    conn.close()
